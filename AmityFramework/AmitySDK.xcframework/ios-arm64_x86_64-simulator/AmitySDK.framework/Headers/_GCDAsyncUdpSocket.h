...
typedef NS_ERROR_ENUM(_GCDAsyncUdpSocketErrorDomain, _GCDAsyncUdpSocketError) {
	_GCDAsyncUdpSocketNoError = 0,          // Never used
	_GCDAsyncUdpSocketBadConfigError,       // Invalid configuration
	_GCDAsyncUdpSocketBadParamError,        // Invalid parameter was passed
	_GCDAsyncUdpSocketSendTimeoutError,     // A send operation timed out
	_GCDAsyncUdpSocketClosedError,          // The socket was closed
	_GCDAsyncUdpSocketOtherError,           // Description provided in userInfo
};
 ...
 * Example:
 * 
 * _GCDAsyncUdpSocketReceiveFilterBlock filter = ^BOOL (NSData *data, NSData *address, id *context) {
 * 
...
/**
 * _GCDAsyncUdpSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, _GCDAsyncSocket will automatically create its own socket queue.
 ...
 * 
 * This means _GCDAsyncUdpSocket automatically supports both protocols,
 * and can send to IPv4 or IPv6 addresses,
 * as well as receive over IPv4 and IPv6.
 * 
 * For operations that require DNS resolution, _GCDAsyncUdpSocket supports both IPv4 and IPv6.
 * If a DNS lookup returns only IPv4 results, _GCDAsyncUdpSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, _GCDAsyncUdpSocket will automatically use IPv6.
 ...
 * This is due to the fact that _GCDAsyncUdpSocket does NOT copy the data.
 ...
 *    or the system tools to handle this aren't available (e.g. on a mobile device).
 * 
 * For more information about _GCDAsyncUdpSocketSendFilterBlock, see the documentation for its typedef.
 ...
 * 
 * Important Note:
 * _GCDAsyncUdpSocket may be running in parallel with your code.
 * That is, your delegate is likely running on a separate thread/dispatch_queue.
 * When you invoke this method, _GCDAsyncUdpSocket may have already dispatched delegate methods to be invoked.
 ...
 * [udpSocket setReceiveFilter:filter withQueue:myParsingQueue];
 * 
 * For more information about _GCDAsyncUdpSocketReceiveFilterBlock, see the documentation for its typedef.
 * To remove a previously set filter, invoke this method and pass a nil filterBlock and NULL filterQueue.
 * 
 ...
 * 
 * The _GCDAsyncUdpSocket instance may optionally be used again.
 ...
 * 
 * Generally _GCDAsyncUdpSocket doesn't use CFStream. (It uses the faster GCD API's.)
