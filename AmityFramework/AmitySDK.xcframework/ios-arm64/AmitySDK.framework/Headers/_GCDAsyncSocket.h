...
@class _GCDAsyncSocketPreBuffer;
...
#endif
#define _GCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define _GCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define _GCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
...
typedef NS_ERROR_ENUM(_GCDAsyncSocketErrorDomain, _GCDAsyncSocketError) {
	_GCDAsyncSocketNoError = 0,           // Never used
	_GCDAsyncSocketBadConfigError,        // Invalid configuration
	_GCDAsyncSocketBadParamError,         // Invalid parameter was passed
	_GCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	_GCDAsyncSocketReadTimeoutError,      // A read operation timed out
	_GCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	_GCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	_GCDAsyncSocketClosedError,           // The remote peer closed the connection
	_GCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
/**
 * _GCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, _GCDAsyncSocket will automatically create it's own socket queue.
...
/**
 * Create _GCDAsyncSocket from already connect BSD socket file descriptor
**/
 ...
 * 
 * For accepting incoming connections, this means _GCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means _GCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, _GCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, _GCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of _GCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a _GCDAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a _GCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that _GCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 *     So _GCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - _GCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default _GCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct _GCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, _GCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by _GCDAsyncSocket,
 ...
 *     You MUST use manual trust evaluation instead (see _GCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use _GCDAsyncSocketSSLProtocolVersionMin & _GCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * _GCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(_GCDAsyncSocket *)sock
 * {
 ...
 * 
 * Note: If the _GCDAsyncSocket instance is deallocated while it is still connected,
 ...
 * - _GCDAsyncSocketManuallyEvaluateTrust == YES
 *
